...
const PatientRegister = () => {
	...
	return (
		<div className="register-shell">
			...
			{message.text && (
				<section
					className={`register-alert register-alert-${message.type}`}
					role="status"
					aria-live="polite"
				>
					<span>{message.text}</span>
					{message.type === 'success' && createdPatientId ? (
						<button
							type="button"
			...
			<form className="register-form" onSubmit={handleSubmit} noValidate>
				<div className="register-field register-field-full">
					...
					<div className="register-picture-wrap">
						...
						/>
						{profilePreview ? (
							<img src={profilePreview} alt="Profile preview" className="register-picture-preview" />
						) : (
							<small className="muted">No image selected.</small>
						)}
					</div>
					{profilePicture && (
